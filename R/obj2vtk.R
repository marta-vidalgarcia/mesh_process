### obj2vtk - FUNCTION TO BATCH-CONVERT STL meshes to VTK meshes
...
#'
#' @param files Optional parameter. The default is all the OBJ meshes in the current directory.
#' @param ID optional vector with IDs used for saving the VTK meshes. Default is the same name as the OBJ meshes
...
obj2vtk <- function(files = NULL, ID = NULL, vtk_path = NULL, print.progress = TRUE){
  if (is.null(files) == TRUE){
    files_list <- dir(getwd(), pattern = "*.obj")
  } else {
    files_list <- files
  }
